...
def test_frame_classification(
    classification_data: Tuple[pd.DataFrame, pd.DataFrame, pd.Series, pd.Series],
) -> None:
    """Test FRAME feature selection for classification."""
    X_train, X_test, y_train, y_test = classification_data
    classifier_model = LogisticRegression(max_iter=200)
    frame_selector = FRAMESelector(model=classifier_model, num_features=2, top_k=4)
...
def test_frame_regression_extended(
    regression_data: Tuple[pd.DataFrame, pd.DataFrame, pd.Series, pd.Series],
) -> None:
    ...
    frame_selector = FRAMESelector(model=regressor_model, num_features=3, top_k=8)
