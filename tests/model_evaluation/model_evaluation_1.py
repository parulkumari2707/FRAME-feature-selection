...
def lasso_selection(
    X: pd.DataFrame, y: pd.Series, num_features: int = 5, alpha: float = 0.02
) -> np.ndarray:
...
def mutual_information_selection(
    X: pd.DataFrame, y: pd.Series, num_features: int = 5
) -> np.ndarray:
...
def run_test_dataset(
    name: str, filepath: str, target_col: Optional[str], header: int = 0, num_features: int = 5
) -> None:
    """
    ...
    try:
        ...
        if pd.api.types.is_object_dtype(y) or (
            np.issubdtype(y.dtype, np.integer) and len(np.unique(y)) < 20
        ):
                    ...
                    results.append(
                        {
                            ...
                            "Selected Features": ", ".join(selected_features[:10])
...
datasets: Dict[str, Tuple[str, Optional[str], int]] = {
    ...
    "Synthetic Baseline 1 ": (
        "data/synthetic_data/synthetic_dataset_Baseline_dataset.csv",
        "target",
        0,
    ),
    "Synthetic High Sparse High Redundancy 1": (
        "data/synthetic_data/synthetic_dataset_High_sparsity,_high_redundancy.csv",
        "target",
        0,
    ),
    "Synthetic High Sparse Low Redundancy 1": (
        "data/synthetic_data/synthetic_dataset_High_sparsity,_low_redundancy.csv",
        "target",
        0,
    ),
    "Synthetic Low Sparse High Redundancy 1": (
        "data/synthetic_data/synthetic_dataset_Low_sparsity,_high_redundancy.csv",
        "target",
        0,
    ),
    "Synthetic Low Sparse Low Redundancy 1": (
        "data/synthetic_data/synthetic_dataset_Low_sparsity,_low_redundancy.csv",
        "target",
        0,
    ),
}
