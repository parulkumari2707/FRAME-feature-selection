...
@pytest.fixture
def valid_data():
    # Generates valid, reproducible data with 50 samples and 10 features.
    rng = np.random.RandomState(42)
    X = pd.DataFrame(rng.randn(50, 10), columns=[f"feature_{i}" for i in range(10)])
    y = pd.Series(rng.randn(50))  # Ensure y is a pandas Series
    return X, y
...
def test_fit_transform(valid_data):
    ...
    selector = FRAMESelector(
        model=GradientBoostingRegressor(random_state=0), num_features=5, top_k=8
    )
